...
class ActionLeaderboard(CmdlineAction):
	def _print_cli(self):
		for leaderboard in self._leaderboards:
	...
	def _export_json(self):
		json_data = {
			"export_ts": datetime.datetime.now(datetime.UTC).isoformat(),
			"leaderboards": [ leaderboard.to_dict() for leaderboard in self._leaderboards ],
		}
		with open(self._args.export_json, "w") as f:
			json.dump(json_data, f)

	def run(self):
		self._leaderboards = [ Leaderboard(db = self._db, collection_name = collection_name) for collection_name in self._args.collection_name ]
		if self._args.export_json is None:
			self._print_cli()
		else:
			self._export_json()
