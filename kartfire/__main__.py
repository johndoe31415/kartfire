...
def main():
	...
	def genparser(parser):
		...
		parser.add_argument("-o", "--sort-order", choices = [ "source", "datetime", "author", "pass", "fail" ], default = "source", help = "Sort by this column. Can be one of %(choices)s, defaults to %(default)s.")
		parser.add_argument("-l", "--limit", metavar = "count", type = int, default = 100, help = "When limiting the amount of shown results, limit to this value. Defaults to %(default)d.")
	...
	def genparser(parser):
		parser.add_argument("-e", "--export-json", metavar = "filename", help = "Export the data into the given JSON file instead of printing it on stdout.")
		...
		parser.add_argument("-D", "--database-filename", metavar = "file", default = "kartfire.sqlite3", help = "Database filename to use. Defaults to %(default)s.")
		parser.add_argument("-v", "--verbose", action = "count", default = 0, help = "Increases verbosity. Can be specified multiple times to increase.")
		parser.add_argument("collection_name", nargs = "+", help = "Test collection name(s) to judge, possibly separated by commas.")
